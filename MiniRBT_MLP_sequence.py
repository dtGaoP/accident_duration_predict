...
def remove_1_from_texts(texts):
    time_split_pattern = r'(-\s*(\d{1,2}:\d{2}|\d{1,2}\.\d{2}))'  # 匹配时间范围的后半部分  # 匹配时间范围的后半部分及之后的任何内容  # 匹配 '- 10:20', '-10:20', ' 10:20', '10:20' 等形式
...
#     scheduler.step()
#     torch.save(model.state_dict(), 'Bert_mlp_model_time_window_256.pth')
...
class DynamicTestLoader:
    def __init__(self, text_data, data, durations, tokenizer, batch_size=16, max_length=256):
        ...
        self.max_length = max_length
        self.time_windows = list(range(0, 191, 10))  # 从0到190，步长为10
...
#加载最佳模型权重
print('----------loading model---------------')
model.load_state_dict(torch.load('Bert_mlp_model_time_window_256.pth'))
model.eval()
test_preds = []
test_labels = []
dynamic_loader = DynamicTestLoader(test_data_text, test_data, test_duration, tokenizer, batch_size=batch_size, max_length=256)
#dynamic_loader = DynamicTestLoader(train_data_text, train_data, train_duration, tokenizer, batch_size=batch_size, max_length=256)
...
for test_loader, window_min in dynamic_loader:
    ...
    # 指定保存的文件名和路径
    csv_file_path = 'predictions_vs_actuals_180.csv'
...
# results_df.to_csv('window_metrics.csv', index=False)
